...
for split in global_params["audio_splits"]:
    ...
    audio_logmel = os.path.join(global_params["dataset_dir"], f"{split}_audio_logmels_clap.hdf5")
    ...
    with h5py.File(audio_logmel, "w") as stream:
        ...
        for fpath in glob.glob(r"{}/*.wav".format(audio_dir)):
            ...
            audio,sr = librosa.load(fpath,sr=48000)
            inputs = feature_extractor(audio,sampling_rate=48000, return_tensors="pt")
            inputs = inputs.to(device)
            emb = model.get_audio_features(**inputs)
            #print('type of emb',type(emb))
            stream[fid] = emb.cpu().detach().numpy()  # [Time, Mel]
            print(fid, fname)
