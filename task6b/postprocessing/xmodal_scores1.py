...
# Trial info
trial_base = "/home/ubuntu/dcase2023-audio-retrieval/"
trial_series = "output/"
trial_name = "2023-04-11_2f0aa_00000/"
ckp_dir = "checkpoint_14"
...
for name, ds in zip(["train", "val", "eval"], [train_ds, val_ds, eval_ds]):
    ...
    print("Computing pairwise cross-modal scores for ", name)
    ...
    with shelve.open(filename=score_fpath, flag="n", protocol=2) as stream:
        for fid in ds.text_data["fid"].unique():
            ...
            print("Entering for loop for ", fid)
            for tid in text2vec:
